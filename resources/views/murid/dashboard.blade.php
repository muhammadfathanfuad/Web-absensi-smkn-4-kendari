...
@section('css')
    @vite(['resources/css/murid/dashboard.css'])
@endsection
...
@push('scripts')
    {{-- Inject data ke window object untuk digunakan oleh JavaScript --}}
    <script>
        @php
            $defaultWinrate = [0, 0, 0, 0, 0, 0, 0];
            $finalWinrate = $winrateData ?? $defaultWinrate;
        @endphp
        window.dashboardMuridData = {
            winrateData: @json($finalWinrate)
        };
    </script>
    @vite(['resources/js/murid/dashboard-murid.js'])
@endpush